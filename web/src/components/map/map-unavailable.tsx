  ...
  return (
    <div className="flex h-full items-center justify-center p-6">
      <div className="region max-w-xl">
        <div className="region-head">No map is being drawn</div>
        <div className="space-y-3 p-4 text-sm text-bone-dim">
          <p>
            This server has map rendering switched off, so there are no tiles for the panel to
            show. It cannot be turned on from here — the game only starts its renderer at
            startup.
          </p>
          <p>To switch it on, edit the server's <code className="readout">serverconfig.xml</code>:</p>
          <pre className="readout overflow-x-auto border border-ash-800 bg-ash-950 p-3 text-2xs">
{`<property name="EnableMapRendering" value="true"/>`}
          ...
          <p className="text-2xs text-bone-faint">
            Ground explored before rendering was switched on stays blank until it is visited
            again, or until an admin runs <code className="readout">visitmap</code> in the
            console to walk the server over it.
          </p>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="pointer-events-none absolute inset-0 z-[500] flex items-center justify-center">
      <div className="region pointer-events-auto max-w-md">
        <div className="region-head">Nothing drawn yet</div>
        <div className="space-y-2 p-4 text-sm text-bone-dim">
          <p>
            Rendering is on, but the server has not drawn any ground yet. Squares appear as
            people explore.
          </p>
          <p className="text-2xs text-bone-faint">
