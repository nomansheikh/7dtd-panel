...
/*
A coordinate no world reaches: the largest map is 16384 blocks, 128 tiles
across at full zoom.
*/
const blankProbe = 1 << 20

/*
A server that answers 404 instead — one with rendering off — leaves this
...
/* Its own type so the probe runs once per game server, not once per process. */
